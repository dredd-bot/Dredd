    ...
    async def on_command_error(self, ctx, exc):
        ...
        if hasattr(ctx.command, 'on_error'):
            ...
            e.description = _("Thank you for using me! Unfortunately this command is vote locked and you'll need to vote for Dredd. You can vote in any of these lists:\n"
                              "`[1]` {0}\n`[2]` {1}\n`[3]` {2}\n`[4]` {3}\n\n*After you vote you'll be able to use this command and will help Dredd gain more servers :D*").format(
        ...
        query = "SELECT DISTINCT error_command, error_occured, error_jump, error_id FROM errors WHERE error_msg = $1 AND error_command = $2 AND error_status = $3"
        error = await self.bot.db.fetch(query, str(tb), str(ctx.command), 0)
        ...
        if len(error) == 0:
            ...
            await self.bot.db.execute("INSERT INTO errors VALUES($1, $2, $3, $4, $5, $6)", str(tb), msg.jump_url, str(ctx.command), 0, datetime.now(), error_id + 1)
