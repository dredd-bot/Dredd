...
def member_activity(ctx, member):
    ...
    for activity in member.activities:
        ...
        if activity.type == discord.ActivityType.custom:
            ...
            message += _("{0} Playing **{1}** ").format(ctx.bot.settings['emojis']['misc']['activity'], clean(activity.name))
            if not isinstance(activity, discord.Game):

                if activity.details:
                    message += _("**| {0}** ").format(escape_markdown(activity.details, as_needed=False))
                if activity.state:
                    message += _("**| {0}** ").format(escape_markdown(activity.state, as_needed=False))
                message += "\n"
            else:
                message += "\n"
            ...
            try:
                ...
                message += _("{0} Streaming **[{1}]({2})** on **{3}**\n").format(
            ...
            message += _("{0} Watching **{1}**\n").format(ctx.bot.settings['emojis']['misc']['activity'], clean(activity.name))

        elif activity.type == discord.ActivityType.listening:

            if isinstance(activity, discord.Spotify):
                url = f"https://open.spotify.com/track/{activity.track_id}"
                message += _("{0} Listening to **[{1}]({2})** by **{3}** ").format(
                    ctx.bot.settings['emojis']['misc']['music'], escape_markdown(activity.title, as_needed=False), url, ', '.join(activity.artists)
                )
                if activity.album and not activity.album == activity.title:
                    message += _(", album — **{0}** ").format(activity.album)
                message += "\n"
            else:
                message += _("{0} Listening to **{1}**\n").format(ctx.bot.settings['emojis']['misc']['music'], clean(activity.name))

        elif activity.type == discord.ActivityType.competing:
            if activity.url is None:
                message += _("{0} Competing in **{1}**\n").format(ctx.bot.settings['emojis']['misc']['activity'], clean(activity.name))
                if activity.emoji:
                    message += f" {activity.emoji}\n"
            elif activity.url is not None:
                message += _("{0} Competing in **[{1}]({2})**\n").format(
                    ctx.bot.settings['emojis']['misc']['activity'], escape_markdown(activity.name, as_needed=False), activity.url
                )
                if activity.emoji:
                    message += f" {activity.emoji}\n"
    if message == '\n\n':
...
def member_status(ctx, member):
    status = {
        "online": f"""{f"{ctx.bot.settings['emojis']['misc']['mobile-online']}" if member.is_on_mobile() else f"{ctx.bot.settings['emojis']['misc']['pc-online']}"}""",
        "idle": f"""{f"{ctx.bot.settings['emojis']['misc']['mobile-idle']}" if member.is_on_mobile() else f"{ctx.bot.settings['emojis']['misc']['pc-idle']}"}""",
        "dnd": f"""{f"{ctx.bot.settings['emojis']['misc']['mobile-dnd']}" if member.is_on_mobile() else f"{ctx.bot.settings['emojis']['misc']['pc-dnd']}"}""",
        "offline": f"{ctx.bot.settings['emojis']['misc']['offline']}"
    }
    ...
    if member.activities:
        ...
        for activity in member.activities:
            if activity.type == discord.ActivityType.streaming:
                ustatus += f"{ctx.bot.settings['emojis']['misc']['streaming']}"
    else:
        ustatus = f'{status[str(member.status)]}'
